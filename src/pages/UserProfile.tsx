...
const UserProfile = () => {
    ...
    const getUserData = async () => {
        try{
            ...
            if(!token){
                setUserDataError("Token id not defined");
                setLoading(false);
                return;
            }
            ...
            setUserDataError("Fetching data failed.")
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <>
            ...
            {loading ? (
                ...
                <div>Error: {userDataError}</div>
            ) : (
                <div className="max-w-lg mx-auto my-10 bg-white rounded-lg shadow-md p-5">
                    <img
                        className="w-32 h-32 rounded-full mx-auto"
                        src={userData ? userData.image || "https://picsum.photos/200" : "https://picsum.photos/200"}
                        alt="Profile picture"
                    />
                    <h2 className="text-center text-2xl font-semibold mt-3">{userData?.username}</h2>
                    <h3 className="text-center text-xl font-semibold mt-3">{userData?.email}</h3>
